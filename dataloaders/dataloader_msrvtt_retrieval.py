...
class MSRVTT_DataLoader(Dataset):
    ...
    def __init__(
        ...
        self.rawVideoExtractor = RawVideoExtractor(framerate=1)
    ...
    def _get_text(self, video_id, sentence):
        ...
        embed = clip.tokenize(sentence)
        text_features = model.encode_text(text)

        return text_features

    def _get_rawvideo(self, choice_video_ids):
        embeds_video = np.zeros(
            (len(choice_video_ids), 400))
        for id, item in enumerate(choice_video_ids):
            embed = np.load("Features/video" + str(item) + ".npy")
            embeds_video[id] = embed
        return embeds_video
